	...
	public function addAudit()
	{
		try {
			...
			for ($i=0; $i < count($part_meta); $i++) { 
			    ...
			    if(Input::get('ranged')==true) DB::select( DB::raw("UPDATE pieces SET created_at='{$ctimestamps} 00:00:00', updated_at='{$ctimestamps} 00:00:00' WHERE id={$piece->id}") );
			...
			if($done) {
			    ...
			    if(Input::get('ranged')==true) DB::select( DB::raw("UPDATE audits SET created_at='{$ctimestamps}', updated_at='{$ctimestamps}' WHERE id={$audit->id}") );
